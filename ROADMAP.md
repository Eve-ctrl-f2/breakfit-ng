...
| ✓ | **Pre-test hardening** | SwUpdate "new version available" reload toast (sticky) + periodic/on-focus update checks — stops testers seeing stale cached builds. Global error reporting enhanced with stack + route (posts to /telemetry/error). |
| ✓ | **Light mode fix** | Root cause: preset defined only `colorScheme.dark`; PrimeNG cards fell back to Aura's dark surface scale in light mode (dark card + dark text). Added a full `light` colorScheme and switched card/dialog background from `{surface.700}` to the per-scheme `{content.background}`. Dark mode unchanged. |
| ◑ | **UX quick wins (audit)** | Done: Stop de-emphasized (no longer destructive-red), break-phase ring uses a calm cyan `--rest` hue (focus keeps the chosen accent), muted-text contrast lifted toward AA, 44px touch targets (bottom-nav, reps), completion-rate hidden until ≥5 breaks, "Save as preset" relabel. Deferred: notifications grouping, stop-undo toast. |
...
| ⏭ | **Apple Health / Health Connect (v2)** | Deferred to a post-v1 feature. Plugin scaffolding + native code already in `native/`; wire it (capability/permissions) when v2 begins. The `window.bfHealth` seam + TCX export cover the meantime. |
| ◑ | **Local notifications (native)** | Web-side done: `NativeReminderService` + dependency-free `local-notifications.ts` schedule a break reminder at phase end (fires backgrounded/closed); no-op on web. Native step: `npm i @capacitor/local-notifications` + `cap sync` (see CAPACITOR.md §7). |
